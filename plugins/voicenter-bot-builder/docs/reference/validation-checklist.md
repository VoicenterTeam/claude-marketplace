...
| V-C2 | **PASS** | 1.20.1 / 1.20.4 | 2026-08-16 | Claude Code (win32) | Delegation engaged (real `spec-verifier` Agent call); report contract-valid; 26 rows, 0 blocking. **Byte-comparability holds**: the force-inline leg (V-C4 step 4, a real Skill 3 assembly at 1.20.4) reproduced `expected-output-shipping.json` **identically — 1140/1140 lines, zero diffs** after path-scoped TS normalization that preserved the 10 frozen `ParameterType.CreatedDate` values. ⚠️ The delegated leg was scored against a *hand* assembly which stripped the `[default — not user-authored]` marker from `prompts.chatInstructions` on the assembler's own judgement, producing a spurious 1139/1140. Skill 3 and the golden agree; the hand assembly was the outlier. The marker leak is real and fixed in 1.20.2, but the frozen fixture/golden pair is self-consistent, so **no re-freeze is required for this gate**. |
...
| V-C4 | **PASS** | 1.20.4 | 2026-08-16 | Claude Code (win32), `agents/`→`agents.off/`, restarted, `Agents (0)` confirmed | **Leg 1 (F1):** inline assembly is an **exact match** to `expected-output-shipping.json` including key order, after path-scoped TS normalization; 26/26 pass, 0 blocking. **Leg 2 (F2):** every blocking verdict identical to delegated run 3 — CHK-03 FAIL `(-12,-999)`, CHK-07 FAIL `caller_full_name`, CHK-05 **pass** (`inline=-999, equal=True`), CHK-02 **6** relations, CHK-22 **FAIL** on `(-12,-18)`, no JSON emitted. **Equivalence holds; no blocking-verdict difference.** Two advisory divergences, inline correct on both and matching the frozen baseline — CHK-08 banded-in-band (`pass` inline vs `FAIL` delegated) and CHK-24 advisory-half scoping (`pass` inline vs `FAIL` delegated). Both fixed in 1.20.5. Leg 1 also independently reproduced the §6.5 ID-scheme drift note. `agents/` restored, `Agents (1)` / ~551 tok verified. |
