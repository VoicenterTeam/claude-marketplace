...
| 3 | **JSON Assembler** | A mechanical projection of the finished spec into wire-format JSON, gated behind a 24-check verification pass. |
...
Runs the 24-check pass in an isolated read-only context and returns a pass/fail report with
...
- **Subagent verification is a Claude Code / Cowork enhancement.** There, the 24-check pass can
...
| `expected-output.json` | That spec's exact assembly output |
