...
This repository ships **two Claude Code plugins** that integrate with the Voicenter telephony platform:
...
docs/
  ...
  skills/<skill-name>/README.md     ← deep reference per skill (15 total)
README.md   CHANGELOG.md   DEMO.md   LICENSE
```

The 15 skills (1 in `voicenter-mcp`, 14 in `voicenter-api`):

> `setup` · `voicebot` · `click2call` · `popup-screen` · `cdr-notification` · `external-layer` · `call-log` · `blacklist` · `mute-recording` · `extension-list` · `real-time` · `productive-dialer` · `login-logout` · `lead-tracker` · `active-calls`
...
- Follow the section order: *When to use → Environment Variables → Endpoint → Authentication → Request → Response → Error codes → TypeScript → Tips → Related Skills*.
...
1. Create `plugins/voicenter-api/skills/<skill>/SKILL.md` with the required frontmatter and section order.
2. Create the matching `docs/skills/<skill>/README.md` (deep reference).
3. Add the row to the skill index in [docs/README.md](docs/README.md), [docs/architecture.md](docs/architecture.md), and [docs/plugins/voicenter-api.md](docs/plugins/voicenter-api.md).
4. Cross-link from related skill pages (`Related skills` section).
5. Update the `description` in `plugins/voicenter-api/.claude-plugin/plugin.json` to reflect the new total.
...
1. Update all three `version` fields (marketplace + both plugins).
2. Add a CHANGELOG entry.
3. Confirm `/plugin` after install still reports both plugins as **Enabled** with the right skill counts (1 + 14).
...
- `/plugin install voicenter-mcp@voicenter` and `voicenter-api@voicenter`
- `/plugin` should show both **Enabled**.
