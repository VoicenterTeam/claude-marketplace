...
| Field | Meaning |
|---|---|
| `announcement` | What the bot says when the API succeeds (v1.5.0 — was `apiResponseAnnouncement` pre-v1.5.0) — **and then it yields the turn and WAITS for a caller answer (v1.17.0 turn-yield fact, FP-3)**. Authored ONLY when the intent's `**Asks next:**` is a question (read-back + that question, almost always with Mustache references against section 4.5.4 dotted paths). When `**Asks next:**` is `[none]` (auto-chaining), this MUST be the empty string — the success speech lives as FP-4 quoted lines in the post-execution `intentInstructions`, immediately before the forward. |
...
Iron rules: every RT=2 intent must have a complete `api_silence_behaviour` (six components: the three language fields `silence_sentence` / `silence_ending_sentence` / `silence_instructions` authored here, plus the three structural fields `silence_duration` / `silence_loops` / **fallback intent** owned by Skill 1 in section 4) and have non-empty `fail_output` / `function_output` / `response_success`; `announcement` is non-empty when `**Asks next:**` is a question and MUST be the empty string when `**Asks next:**` is `[none]` (v1.17.0, FP-3 turn-yield — check 10's rewritten announcement clause). The **fallback intent** is the failover Skill 3 resolves into `api_silence_behaviour.intent` and `apiSilenceRelations[].ApiSilenceIntentID` — if it's missing/unresolved in section 4, check 11 halts and routes back to Skill 1 patch mode (an RT=2 intent without it has no failover when the caller goes silent mid-API). For `function_output`, the object `{ "default": "<fallback>" }` qualifies as non-empty. For `response_success`, the object `{ "instructions": "" }` (empty inner string) qualifies. Per §14.3.6, missing silence behavior produces dead air at runtime when the API takes 8+ seconds.
...
**Question-less announcement rule (v1.17.0, FP-3 turn-yield — blocking):** an RT=2/RT=3 `announcement` on an intent whose `**Asks next:**` is `[none]` must be the empty string, regardless of content — the announcement is a wait-for-answer directive, and on an auto-chaining intent it stalls the call into the silence loop ("האם אתם עדיין על הקו?"). Remaining spoken lines move to FP-4 quoted lines in the post-execution `intentInstructions` immediately before the forward instruction; the explicit wait rule is likewise NEVER authored on auto-chaining intents — they get `Immediately forward the call to <next intent's Description>, without waiting for a response from the customer.` instead. Verified by Skill 3 check 24.
...
| 10 | RT=2 only: `fail_output`, `function_output` (object `{ "default": "..." }`), `response_success` (object `{ "instructions": "..." }`) all populated; `announcement` populated when `**Asks next:**` is a question and **empty when `**Asks next:**` is `[none]`** (v1.17.0, FP-3 turn-yield) |
