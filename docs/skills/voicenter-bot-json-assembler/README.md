...
**Operating principle: pure parser, not interpreter.** Skill 3 makes no creative decisions. If the spec deviates from the strict template, Skill 3 emits a structured parse error and refuses to assemble. If the §15.4 cross-reference pass fails any of **twenty-two checks** (eight §15.4 + three Compass + three botIntents-role + one duplicate-global-intent + seven field-placement doctrine — v1.13.0), Skill 3 emits a structured failure report with routing recommendations and refuses to emit JSON. Checks 1–7, 11–13, 15, and 16–21 are blocking; 14 and 22 are advisory. The discipline is the design — if Skill 3 interpreted, "what JSON does this spec produce?" would depend on Skill 3's mood, and the source-of-truth contract dies.
...
## §15.4 cross-reference pass — twenty-two checks

After assembly + section 6 sanity check, run all **twenty-two** checks against the in-memory wire structure — eight per Doc 1 §15.4, three Compass doctrine checks (8–10), three botIntents-role integrity checks (11–13, v1.8.0), one duplicate-global-intent check (15, v1.12.0), and **seven field-placement doctrine checks (16–22, v1.13.0)** per `plugins/voicenter-bot-builder/references/field-placement-doctrine.md`. Checks 1–7, 11–13, 15, and 16–21 are blocking and run unconditionally so the user gets a complete failure report rather than fixing one issue at a time. Check 8 is advisory/blocking by token band, check 9 is advisory, check 10 is blocking on mismatch — all three gated on Gemini 3.1 (`AiModelConfig.created.model = "models/gemini-3.1-flash-live-preview"`, i.e. `AIModelConfigID=139` or `142`). Checks 14 and 22 are non-blocking advisory. Checks 11–13, 15, and 16–22 are model-agnostic. (The v1.8.0 fan-out-completeness check was removed in v1.12.0 when global fan-out was dropped.)

Run order: 1 → 2 → 3 → 4 → 5 → 6 → 7 → 11 → 12 → 13 → 15 → 16 → 17 → 18 → 19 → 20 → 21 → 22 → 8 → 9 → 10 → 14. The pass operates on the assembled in-memory wire structure; checks 16–21 additionally consult the spec's section-4 staggering/terminal fields and the 4.5 variable inventory. The failure report is count-agnostic: `Checks failed: <count> of <total checks run>`.
