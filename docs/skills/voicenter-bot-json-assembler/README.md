...
After assembly + section 6 sanity check, run all **twenty-two** checks against the in-memory wire structure — eight per Doc 1 §15.4, three Compass doctrine checks (8–10), three botIntents-role integrity checks (11–13, v1.8.0), one duplicate-global-intent check (15, v1.12.0), and **seven field-placement doctrine checks (16–22, v1.13.0)** per `plugins/voicenter-bot-builder/references/field-placement-doctrine.md`. Checks 1–7, 11–13, 15, and 16–21 are blocking and run unconditionally so the user gets a complete failure report rather than fixing one issue at a time. Check 8 is advisory/blocking by token band, check 9 is advisory, check 10 is blocking on mismatch — all three gated on Gemini Live 3.1 (`AiModelConfig.created.model = "models/gemini-3.1-flash-live-preview"`). Checks 14 and 22 are non-blocking advisory. Checks 11–13, 15, and 16–22 are model-agnostic. (The v1.8.0 fan-out-completeness check was removed in v1.12.0 when global fan-out was dropped.)
