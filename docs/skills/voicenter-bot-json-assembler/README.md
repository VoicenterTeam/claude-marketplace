...
**Version-level `ActiveVersionInfo.AIModelConfig` fields (v1.5.0, extended v1.13.0):** `max_duration` (spec section 1 `**Max call duration:**`, default 1200), `daily_limit` (spec section 1 `**Daily limit:**`, integer, default 600 — v1.13.0 golden-export field), `dailyLimitLayerId` (spec section 1 `**Daily limit layer:**`, integer layer ID, default 3 — v1.13.0), `maxDurationLayerId` (spec section 1 `**Max duration layer:**`, integer layer ID, **default `0` — v1.14.0, was 3**; Skill 1 asks via the MCP layer list when connected), `daily_limit_sentence` (spec section 1 `**Daily limit sentence:**`; production-derived default `"Sorry, but reached daily limit of calls duration, please try again later or contact the copany's support"` — v1.13.0), `max_duration_sentence` (spec section 1 `**Max duration sentence:**`; production-derived default **`"נראה שהגענו לזמן שיחה מקסימלי, אנא נסה שנית "`** — v1.14.0, verbatim including the trailing space; replaces the v1.13.0 English string), `IVRLayerSelect_2` (spec section 1 `**IVRLayerSelect_2:**`, integer, default 3 — v1.13.0), `prompts` (five-field bundle), `recordAgentCalls` (spec section 1 `**Record agent calls:**` emitted as STRING `"false"`/`"true"`), `silence_behaviour` (conditional on section 3; **v1.8.0:** carries a structural `intent` failover as its first key — the resolved `IntentId` to jump to on caller-silence exhaustion, defaulting to the transfer-to-human global; the bot-level analogue of `api_silence_behaviour.intent`), `created` (lean payload above). Fields `tools: []` and `instructions: ""` removed from this level.
...
|---|---|---|
| `max_turns` | `5` for **all** RTs (v1.14.0 — replaces the pre-v1.14 RT=2 `15` rule) | Section 4 `**Max turns:**` — Skill 1 sets `10` autonomously on conversation-heavy intents |
| `sensitive` | `false` (JSON boolean) | Section 4 `**Sensitive:**` (v1.14.0: `true` only on sensitive-collecting intents) |
| `max_turns_sentence` | `"מתנצל אבל נראה שיש לי בעיה מסויימת, אנא נסה שנית מאוחר יותר"` for **all** RTs (v1.14.0 masculine fallback) | Section 4 `**Max turns sentence:**` — Skill 2 normally authors the gender-matched sentence there |

**This replaces the pre-v1.13 sibling emission block: never emit `max_turns` / `max_turns_sentence` as direct siblings of `prompts` inside `IntentConfig` (the pre-v1.13 shape)** — they live inside `additional` together with `sensitive`. When a default is applied, it is listed once in the banner DEFAULTS APPLIED section (aggregated, not per-intent). **v1.14.0 replaced the v1.5.0 "RT=2 defaults to 15" design decision 6 with a uniform `5`:** both v1.14.0 reference exports (transport route-planner; supermarket cart agent) carry `max_turns: 5` on nearly every intent regardless of RT, with `10` only on conversation-heavy intents. The `10` upgrade is Skill 1's autonomous call via section 4 `**Max turns:**`; spec authors can still override per intent there.
...
|---|---|
| 1 | `layer` (**JSON integer, never a quoted string** — see the layer-typing rule below), `intentLoadingAnnouncement` (always emitted) — and **no `announcement` key at all** (v1.14.0, Appendix A row 24: the farewell lives in the predecessor's `intentInstructions` per the FP-8 trigger rule; a spec-supplied RT=1 announcement is a check-20 failure, not an emission choice). Terminal doctrine (v1.13.0, FP-8) — one RT=1 terminal per outcome, owning its outcome slot, no terminal→anything relations — is validated by cross-reference check 20. |
| 2 | In emission order: `url`, `method`, `headers`, `body` (key omitted entirely when the spec declares no `**Body:**`), `fail_output`, `announcement` (v1.5.0 — was `apiResponseAnnouncement`), `function_output` (object `{ "default": "..." }` — v1.5.0), `response_success` (object `{ "instructions": "..." }` — v1.5.0), `intentInstructions`, `api_silence_behaviour` (six-key object — see below), `intentLoadingAnnouncement` (lowercase only — capital-I `IntentLoadingAnnouncement` REMOVED) |
...
| 4 | `phone1`, `phone2`, `phone3`, `parameter_phone` (when slot-driven), `selectdial_option`, `NEXT_VO_ID`, `MAX_DIAL_DURATION`, `record`, `announcement`, `intentLoadingAnnouncement`, `intentInstructions`, `response_success` (object `{ "instructions": "..." }`). **Sourcing:** the first eight are structural and come from section 4's RT-specific block; the last four are language content and are read from **section 5** first, falling back to the optional section-4 labels of the same name only when section 5 has no entry. |

### Layer typing and the two portable layer IDs (v1.20.1)

`Configuration.layer`, `dailyLimitLayerId`, `maxDurationLayerId`, `IVRLayerSelect_2` and `NEXT_VO_ID` are emitted as **bare JSON numbers** — `"layer": 666`, never `"layer": "666"`. The `ImportBotFromJSON` contract's §2 Types list names the fields it requires as numbers and **does not include any of these**, so nothing upstream pins them; they are correct today only by construction. The hazard is proximity — `recordAgentCalls: "false"` and `realtimeInputConfig.automaticActivityDetection.disabled: "true"` are deliberately string-typed (Appendix A rows 17–18) and live in the same objects, which makes "these platform flags are strings" an easy over-generalization when hand-authoring.

Two layer IDs are portable across accounts and are Skill 1's no-preference defaults: **`666`** (the built-in hang-up layer, present on every account) for terminals that end the call, and **`0`** (the first layer created on every account) for human-transfer terminals. Skill 3 emits whatever the spec carries and never substitutes either value; it banner-notes account-specific layers for post-import verification.

**Diagnostic note.** A quoted layer and a layer that doesn't exist on the target account produce the *same* visible symptom — the UI's layer dropdown showing the raw ID instead of the layer name. Check the emitted JSON's type before concluding which one you have.
...
## §15.4 cross-reference pass — twenty-six checks

After assembly + section 6 sanity check, run all **twenty-six** checks against the in-memory wire structure — eight per Doc 1 §15.4, three Compass doctrine checks (8–10), three botIntents-role integrity checks (11–13, v1.8.0), one duplicate-global-intent check (15, v1.12.0), **nine field-placement doctrine checks (16–24, v1.13.0/v1.14.0/v1.17.0)** per `plugins/voicenter-bot-builder/references/field-placement-doctrine.md`, one **persona-FK sanity check (25, v1.18.0)** per `plugins/voicenter-bot-builder/references/voicebot-json-contract.md` R7/R11, and one **layer-typing check (26, v1.20.1)** per the same contract's §2 Types. Checks 1–7, 11–13, 15, 16–21, 24, and 26 are blocking and run unconditionally so the user gets a complete failure report rather than fixing one issue at a time. Check 8 is advisory/blocking by token band, check 9 is advisory, check 10 is blocking on mismatch — all three gated on Gemini 3.1 (`AiModelConfig.created.model = "models/gemini-3.1-flash-live-preview"`, i.e. `AIModelConfigID=139` or `142`). Checks 14, 22, 23, and 25 are non-blocking advisory. Checks 11–13, 15, 16–24, 25, and 26 are model-agnostic. (The v1.8.0 fan-out-completeness check was removed in v1.12.0 when global fan-out was dropped.)
...
| 26 | Layer fields are integers (v1.20.1) | **Blocking.** Every RT=1 `Configuration.layer`, every RT=4 `Configuration.NEXT_VO_ID`, and the version-level `dailyLimitLayerId` / `maxDurationLayerId` / `IVRLayerSelect_2` are JSON numbers, not quoted strings (and not booleans — in many languages `bool` subclasses `int`, so reject it explicitly). A quoted layer is an FK the platform cannot resolve; the bot imports and then the layer dropdown renders the raw ID instead of the layer name. That symptom is identical to a layer that does not exist on the target account, so operators cannot tell the two apart from the UI. The check does NOT verify a layer *exists* — it has no platform access. Remediation: Skill 3 internal bug (the spec parse rules already require integers). |
...
| Check 26 — layer field emitted as a string/bool | Skill 3 internal bug — the emission path stringified a value the spec parse rules already require to be an integer. Report and halt; user files a skill-level issue |
...
#   - ActiveVersionInfo.AIModelConfig.daily_limit = 600, dailyLimitLayerId = 3, IVRLayerSelect_2 = 3 (v1.13.0 golden-derived defaults — layer targets are account-specific; verify after import)
#   - ActiveVersionInfo.AIModelConfig.maxDurationLayerId = 0 (v1.14.0 default — was 3 pre-v1.14; Skill 1 asks via the MCP layer list when connected)
...
This page previously jumped from the v1.13.0 notes straight to the v1.18.0 contract section, so the v1.14.0 defaults were never mirrored here and several statements above went stale. Recorded now; where this section and the v1.5.0 / v1.13.0 sections below disagree, **v1.14.0 wins.**

**`IntentConfig.additional` defaults are uniform.** `max_turns` is `5` for **all** response types — this replaces the v1.5.0 "RT=2 defaults to 15" design decision 6. Both v1.14.0 reference exports (transport route-planner; supermarket cart agent) carry `5` on nearly every intent regardless of RT, with `10` only on conversation-heavy intents; the `10` upgrade is Skill 1's autonomous call via section 4 `**Max turns:**`, never a user prompt. `max_turns_sentence` likewise defaults to the single masculine fallback `"מתנצל אבל נראה שיש לי בעיה מסויימת, אנא נסה שנית מאוחר יותר"` for all RTs (Skill 2 normally authors a gender-matched sentence into section 4), replacing the pre-v1.14 split of an RT=2 Hebrew string and `""` elsewhere.

**Layer-target defaults split.** `dailyLimitLayerId` and `IVRLayerSelect_2` stay at the golden-derived `3`; `maxDurationLayerId` now defaults to **`0`** (was `3`) and is user-chosen via the MCP layer list when Skill 1 is connected. `max_duration_sentence`'s production-derived default is now the Hebrew `"נראה שהגענו לזמן שיחה מקסימלי, אנא נסה שנית "` (verbatim, including the trailing space), replacing the v1.13.0 English string.

**RT=1 carries no `announcement` key** (Appendix A row 24). The farewell lives in the predecessor's `intentInstructions` per the FP-8 trigger rule; the terminal speaks only its short `intentLoadingAnnouncement`. A spec-supplied RT=1 announcement is a check-20 failure, not an emission choice.

**`silence_behaviour.intent` targets a real bot-own intent.** The pre-v1.14 "substitute canonical system global 19" mechanism was removed — silence forwarding always points at an intent the user chose an outcome for, emitted as its negative placeholder with the mandatory post-import banner line (the import procedure does not remap this field).
