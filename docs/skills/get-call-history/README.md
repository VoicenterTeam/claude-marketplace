...
Pull specific call data by CallID with AI analysis — summary, emotions, personality traits, and insights. Does NOT include full transcription (CDR Notification only).

> Source: [`plugins/voicenter-api/skills/get-call-history/SKILL.md`](../../../plugins/voicenter-api/skills/get-call-history/SKILL.md)
...
> **Not the right skill if:** the user needs full transcription. Transcription is only available via **CDR Notification** (push/webhook) — redirect to that skill in that case.
...
**Authentication differs from other Voicenter APIs:** most Voicenter APIs use a `code` query parameter, but GetCallHistory uses `Authorization: Bearer {token}` instead.
...
- [CDR Notification](../cdr-notification/README.md) — push webhook after each call; the only source for full transcription; also returns all AI data automatically
- [Call Log](../call-log/README.md) — use as Step 1 to get CallIDs when doing retroactive phone-number lookups
- [Click2Call](../click2call/README.md) — the `CALLID` in its response is the same ID passed here
- [Real-Time](../real-time/README.md) — the `ivruniqueid` in real-time events is the same ID passed here
